...
const load = async ({ params, fetch: _fetch, url, depends, route }) => {
  ...
  if (list === "section" && rest === "new-releases") {
    ...
    const catInfoJSON = catInfo.ok ? await catInfo.json() : void 0;
    ...
    const artistInfoJSON = artistInfo.ok ? await artistInfo.json() : void 0;
    if (dataType === "albums") {
      request = fetch(
  ...
  const getData = () => {
    if ("items" in resJSON)
      return resJSON;
    if ("playlists" in resJSON)
      return resJSON.playlists;
    if ("albums" in resJSON)
